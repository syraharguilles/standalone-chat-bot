...
export const GET = async ({url, setHeaders}) => {
    ...
    if (requestQueryString.get('title')) {
        urlConsumerCategories += '/title-categories';
        queryString.append('lang', '1');
    } else {
    ...
    if (requestQueryString.getAll('chosenCatId').length > 1) {
        requestQueryString.getAll('chosenCatId').forEach(item => {
            queryString.append('chosenCatId', item);
        });
    }

    if (requestQueryString.get('chosenCatId')) {
        queryString.append('chosenCatId', requestQueryString.get('chosenCatId'));
    }
