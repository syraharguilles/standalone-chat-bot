<script>
    ...
    const addSupCat = (id, description) => {
        const parsedId = parseInt(id);

        if (!postData.supCatIds.includes(parsedId)) {

            // https://svelte.dev/tutorial/updating-arrays-and-objects
            postData.supCatIds = [...postData.supCatIds, parsedId];
            ...
            stateMessage.messages = [
                ...stateMessage.messages, 
                createClientMessage("p", MESSAGES_CHATCLIENT_OBJ_TYPE.SUP_CAT_IDS, message)];
            ...
            const parseObj = {
                id: parsedId, 
                type: MESSAGES_CHATCLIENT_OBJ_TYPE.SUP_CAT_IDS
            }

            MessageParserInstance.parseSubCategoryList(parseObj);
...
<ul class="list-selection">
    ...
    {#each messageObj.messageList as item}
        <li class="list-selection-item" on:click={addSupCat(item.id, item.description)} value={item.id}>{item.description}</li>
    {/each}
</ul>
